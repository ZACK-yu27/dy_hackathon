...
export function RouteBlock({ id, module, onRemove, isOverlay = false }: RouteBlockProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <article
      ref={setNodeRef}
      className={`relative flex overflow-hidden rounded-[18px] bg-[#FCFBF7] shadow-[0px_4px_12px_rgba(27,29,17,0.05)] transition ${
        isDragging ? 'scale-[1.02] opacity-55 shadow-[0px_16px_30px_rgba(27,29,17,0.16)]' : ''
      } ${isOverlay ? 'w-[210px] scale-[1.04] shadow-[0px_18px_40px_rgba(27,29,17,0.22)]' : ''}`}
      style={{
    ...
    >
      ...
      >
        ...
        <span className="absolute -right-2 top-3 h-4 w-4 rounded-full border-[5px] border-[#F1F0E9] bg-white" />
        <span className="absolute -right-2 bottom-3 h-4 w-4 rounded-full border-[5px] border-[#F1F0E9] bg-white" />
      </div>
      ...
      <div className="min-w-0 flex-1 px-3 py-[13px]">
        ...
        {module.riskTags?.length ? (
          <a
            ...
            className="mt-3 inline-flex max-w-full items-center gap-1 rounded-[4px] border border-[#F1B5A8] bg-[#FFE7E0] px-2 py-1 text-[11px] text-[#E05A4E]"
          >
      ...
      {!isOverlay ? (
        ...
        <img src={detailMoreIcon} alt="" className="absolute right-2 top-2 h-[14px] w-[14px] opacity-50" />
      )}
    </article>
  )
}
