...
class Settings(BaseSettings):
    ...
    def apply_kimi_defaults(self) -> "Settings":
        defaults = _load_kimi_defaults(Path(self.api_keys_file))
        if not self.kimi_api_key:
            self.kimi_api_key = defaults.get("kimi_api_key", "")
        if not self.kimi_base_url:
            self.kimi_base_url = defaults.get("kimi_base_url", "https://api.kimi.com/coding/v1")
        if not self.kimi_model:
            self.kimi_model = defaults.get("kimi_model", "kimi-for-coding")
        return self
